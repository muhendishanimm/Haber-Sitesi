...
<div id="tabs">
   ...
   <div><br><br></div>
<%
int id=Integer.parseInt(request.getParameter("YrmID"));
String Onay="Almadi";
ResultSet rs=sayiCrud.yorumselectData(id);
if(rs.next()){
	Onay=rs.getString("Onay");
	session.setAttribute("YrmID",id);
}

%>
 <div id="main">
<div class="container">
  <form action="YorumUpdate_Islem.jsp" method="post">
   <div>
   <label>Yorumun Görünürlük Durumu</label>
	<div> 
       
        <%if(Onay.equals("True")){ %>
         <input name="Onay" type="radio" value="False"/>False
        <input name="Onay" type="radio" value="True"checked="checked"/>True
       
        <%} if(Onay.equals("False")){ %>
        
        <input name="Onay" type="radio" value="False"checked="checked"/>False
        <input name="Onay" type="radio" value="True"/>True
        <%} %>
    </div>
