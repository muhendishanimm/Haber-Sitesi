...
ResultSet rs=sayiCrud.kullaniciupdateselectData(id);
if (rs.next()){
	KullaniciAdi=rs.getString("KullaniciAdi");
	KullaniciMail=rs.getString("KullaniciMail");
...
<body>
  ...
  <form action="AdminSifreUpdate_Islem.jsp">
    ...
    <input type="text" id="KullaniciAdi" value="<%=rs.getString("KullaniciAdi")%> " name="KullaniciAdi" required >
    <label>Kullanıcı Mail</label>
     <input type="text" id="KullaniciMail" value="<%=rs.getString("KullaniciMail")%> " name="KullaniciMail" required>
    <input type="hidden" id="deger"  name="deger" value="1" required>
 <label for="usrname">Eski Sifre </label>
  ...
  <input type="password" id="yeniSifre" name="yeniSifre" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" >
    
      <label for="psw">Yeni Sifre Tekrar  </label>
  <input type="password" id="yeniSifretekrar" name="yeniSifreTekrar">
